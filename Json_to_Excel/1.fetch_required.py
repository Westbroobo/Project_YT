...
required_parameters = input('请输入需要的参数（若有多个请以英文分号隔开）：')
parameters_columns = required_parameters.split(';')
parameters_columns = [param.strip() for param in parameters_columns]

dict_ = {}
df = pd.DataFrame(columns=parameters_columns)
for json_data in Json_Data:
    data_dict = json.loads(json_data)
    for param in parameters_columns:
        value = ''
